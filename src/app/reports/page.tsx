  ...
  return (
    <div className="min-h-screen bg-[#07090e] text-slate-100 flex flex-col">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 flex-1 w-full space-y-6">
        ...
        <div className="p-8 rounded-3xl bg-[#0d101a] border border-cyan-500/30 shadow-2xl space-y-6 relative overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div>
              <span className="text-[10px] font-mono text-cyan-400 uppercase tracking-widest block">
                DOSSIER REF: MENTRA-WK-38
              </span>
              <h2 className="text-xl font-bold text-white mt-0.5">Week 38 Operational Briefing</h2>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800">
              <span className="text-[10px] font-mono text-slate-400 uppercase">LEVEL ADVANCE</span>
              <div className="text-xl font-bold text-cyan-400 mt-1">LV.06 → LV.07</div>
            </div>
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800">
              <span className="text-[10px] font-mono text-slate-400 uppercase">XP HARVESTED</span>
              <div className="text-xl font-bold text-amber-400 mt-1">+870 XP</div>
            </div>
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800">
              <span className="text-[10px] font-mono text-slate-400 uppercase">MISSIONS EXECUTED</span>
              <div className="text-xl font-bold text-emerald-400 mt-1">{completedQuestsCount} / {quests.length}</div>
            </div>
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800">
              <span className="text-[10px] font-mono text-slate-400 uppercase">MONTHLY RUN-RATE</span>
              <div className="text-xl font-bold text-violet-400 mt-1">₹52,000</div>
            </div>
          </div>
